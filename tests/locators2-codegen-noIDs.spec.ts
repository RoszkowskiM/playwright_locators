...
test("test with IDs", async ({ page }) => {
  ...
  await page.getByTestId("dti-range").fill("42");
});

test("test with no IDs", async ({ page }) => {
  await page.goto("http://localhost:3000/practice/simple-elements-no-ids.html");
  ...
  await page.getByRole("slider").fill("42");
});
